...
MatterStatus.transaction do
  ...
  matter_statuses = ["active", "pending", "closed", "appealed", "lapsed", "expired_bt", "reinstated", "canceled"]
...
#    Country.create(:name => country.capitalize.gsub(" ", '_').gsub("(", "").gsub(')', '').gsub("'", "")) unless !Country.find_by_name(country.capitalize).nil?
...
#                        })
#    if @result.save
#      country_name = company[6].upcase.gsub(" ", "_").gsub("'", "").gsub("(", "").gsub(")", "")
#      unless !Address.where(:party_id => @result.company.party_id, :address_type_id => AddressType.find_by_name('BILL_TO').id, :country_id => Country.find_by_name(country_name).id, :city => company[3], :street => company[4], :house_number => company[5]).first.nil?
#        @address = Address.new(:party_id => @result.company.party_id, :address_type_id => AddressType.find_by_name('BILL_TO').id, :country_id => Country.find_by_name(country_name).id, :city => company[3], :street => company[4], :house_number => company[5])
#        @address.save
#      end
#    end
#  end
