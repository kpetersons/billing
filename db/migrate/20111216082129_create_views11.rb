  ...
  def up
   ...
   create view v_matters as (
      ...
      from
        ...
        left outer join (select array_agg(c.code)::text as classes, mc.matter_id from matter_clazzs mc, clazzs c where mc.clazz_id = c.id group by mc.matter_id) cl on ma.id = cl.matter_id,
        customers applicant,
        parties appl_party,
