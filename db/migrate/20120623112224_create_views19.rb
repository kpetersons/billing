class CreateViews19 < ActiveRecord::Migration
  def up
    execute <<-SQL
...
select
   ...
   replace(coalesce(addr.country, '') || ', ' || coalesce(addr.city, '') || ', ' || coalesce(addr.street, '') || ', ' || coalesce(addr.house_number, '') || ', ' || coalesce(addr.room_number, '') || ', ' || coalesce(addr.post_code, '') || ', ' || coalesce(addr.po_box, ''), ', ,', '') as address,
   curr.currency,
