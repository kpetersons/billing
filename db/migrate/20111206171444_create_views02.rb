class CreateViews02 < ActiveRecord::Migration
  def up
    execute <<-SQL
...
select
   ...
   (total_attorney_fee + total_official_fee) * 0.22 as total_vat,
   (total_attorney_fee + total_official_fee) *1.22  as grand_total
from (
	select
	        ...
	        il.total_attorney_fee - nullif(il.total_attorney_fee/100*i.discount, 0) as total_attorney_fee
	      from
	        invoices i	left outer join (
	              ...
	              left outer join (
	              		...
	              		from invoice_lines group by invoice_id, total_discount
