class CreateViews25 < ActiveRecord::Migration
  def up
    execute <<-SQL
        ...
        select
         ...
         from
          ...
          LEFT JOIN (select matter_id, 'Y'::text as image_exists from matter_images group by matter_id) mim ON (ma.id = mim.matter_id)
           ...
           left outer join (select party_id, array_to_string(array_agg(c.name), ' \ ') country from addresses a, countries c where date_effective_end is null and a.suspended = false and a.country_id = c.id group by party_id) ctry_agent on ctry_agent.party_id = agent.party_id
           left outer join (select party_id, array_to_string(array_agg(c.name), ' \ ') country from addresses a, countries c where date_effective_end is null and a.suspended = false and a.country_id = c.id group by party_id) ctry_applicant on ctry_applicant.party_id = applicant.party_id
