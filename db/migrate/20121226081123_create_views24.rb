class CreateViews24 < ActiveRecord::Migration
  def up
    execute <<-SQL
    ...
    execute <<-SQL
      create or replace view v_matters as (
        select
          ...
          t.non_lv_reg_nr,
          t.publication_date,
          ctry_agent.country as country_agent,
          ctry_applicant.country as country_applicant
         from
          ...
          LEFT JOIN (select matter_id, 'Y'::text as image_exists from matter_images group by matter_id) mim ON (ma.id = mim.matter_id)
           ...
           left outer join (
            select appl.party_id, appl.orig_id, appl_c.name from customers appl, companies appl_c where appl.party_id = appl_c.party_id and appl.date_effective_end is null
           ) applicant on applicant.orig_id = ma.applicant_id
           left outer join (
            select ag.party_id, ag.orig_id, ag_c.name from customers ag, companies ag_c where ag.party_id = ag_c.party_id and ag.date_effective_end is null
           ...
           left outer join (select party_id, array_to_string(array_agg(c.name), ' \ ') country from addresses a, countries c where date_effective_end is null and a.suspended = false and a.country_id = c.id group by party_id, c.name) ctry_agent on ctry_agent.party_id = agent.party_id
           left outer join (select party_id, array_to_string(array_agg(c.name), ' \ ') country from addresses a, countries c where date_effective_end is null and a.suspended = false and a.country_id = c.id group by party_id, c.name) ctry_applicant on ctry_applicant.party_id = applicant.party_id
