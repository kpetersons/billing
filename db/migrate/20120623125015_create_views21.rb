...
select
   ...
   trunc(round(case invoice_type = 1 when true then case apply_vat when true then (total_attorney_fee + total_official_fee) * (vat_rate * 100) else (total_attorney_fee + total_official_fee) end else (total_attorney_fee) * (vat_rate * 100) + total_official_fee end, 2), 2) as grand_total,
